...
subarray makes the whole array sorted. The array contains non negative numbers
...
a) Scan from left to right and find the first element which is greater than the next element. Let s be the index of
   such an element. In the above example 1, s is 3 (index of 30).
...
class Solution:
    ...
    def maximum_unsorted_subarray(self, A):
        start = 0
        end = 0
        # Check for the start of the subarray.
        for i in xrange(0, len(A) - 1):
            if A[i] > A[i + 1]:  # Elements not sorted in descending order! This marks the start of my subarray.
                start = i
                break
        if start == 0:  # Array is already sorted.
            return [-1]
        # Check for the end of the subarray.
        for i in xrange(len(A)-1, 0, -1):
            if A[i] < A[i-1]:
                end = i - 1
        # Check if sorting the subarray would make the entire array sorted.
        candidate_subarray = A[start:end+1]
        minimum_candidate = min(candidate_subarray)
        maximum_candidate = max(candidate_subarray)
        for i in xrange(0, start):
            if A[i] > minimum_candidate:
                start = i
                break
        for i in xrange(len(A)-1, end, -1):
            if A[i] < maximum_candidate:
                end = i
                break
...
sol = Solution()
sol.maximum_unsorted_subarray([10, 12, 20, 30, 25, 40, 32, 31, 35, 50, 60])
